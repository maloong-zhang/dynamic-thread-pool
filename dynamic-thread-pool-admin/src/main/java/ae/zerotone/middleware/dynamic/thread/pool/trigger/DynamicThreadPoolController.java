package ae.zerotone.middleware.dynamic.thread.pool.trigger;

import ae.zerotone.middleware.dynamic.thread.pool.sdk.domain.model.entity.ThreadPoolConfigEntity;
import ae.zerotone.middleware.dynamic.thread.pool.types.Response;
...
public class DynamicThreadPoolController {
  ...
  /**
   * 查询线程池数据 curl --request GET \ --url
   * 'http://localhost:8089/api/v1/dynamic/thread/pool/query_thread_pool_list'
   */
  ...
  /**
   * 查询线程池配置 curl --request GET \ --url
   * 'http://localhost:8089/api/v1/dynamic/thread/pool/query_thread_pool_config?appName=dynamic-thread-pool-test-app&threadPoolName=threadPoolExecutor'
   */
  ...
  public Response<ThreadPoolConfigEntity> queryThreadPoolConfig(
      @RequestParam String appName, @RequestParam String threadPoolName) {
    try {
      String cacheKey =
  ...
  /**
   * 修改线程池配置 curl --request POST \ --url
   * http://localhost:8089/api/v1/dynamic/thread/pool/update_thread_pool_config \ --header
   * 'content-type: application/json' \ --data '{ "appName":"dynamic-thread-pool-test-app",
   * "threadPoolName": "threadPoolExecutor", "corePoolSize": 1, "maximumPoolSize": 10 }'
   */
  ...
  public Response<Boolean> updateThreadPoolConfig(@RequestBody ThreadPoolConfigEntity request) {
    try {
      ...
      RTopic topic =
          redissonClient.getTopic("DYNAMIC_THREAD_POOL_REDIS_TOPIC" + "_" + request.getAppName());
